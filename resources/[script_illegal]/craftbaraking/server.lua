...
RegisterServerEvent('craftbaraking:getPlayerInventory')
AddEventHandler('craftbaraking:getPlayerInventory', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	if xPlayer ~= nil then
		TriggerClientEvent('craftbaraking:openMenu', _source, xPlayer.inventory)
	end
end)
...
RegisterServerEvent('craftbaraking:craftItem')
AddEventHandler('craftbaraking:craftItem', function(ingredients)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	...
	if not item then
		...
		if xPlayer ~= nil then
			if hasAllIngredients(xPlayer.inventory, Config.Recipes[item]) then
				...
				TriggerClientEvent('esx:showNotification', _source, '~y~Item crafter: ~w~' .. itemLabel(item, xPlayer.inventory))
			else
				TriggerClientEvent('esx:showNotification', _source, 'Vous n avez pas tout les ingredients')
			end
		end
	end
end)
