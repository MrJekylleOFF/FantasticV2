...
Radio.Labels = {        
    { "FRZL_RADIO_HELP", "~s~~g~Fréquence:~s~ ~1~ MHz" },
    { "FRZL_RADIO_HELP", "~s~~g~Fréquence:~s~ ~1~ MHz" },
    --{ "", "" },
}
Radio.Commands = {
                            ...
                            if not radioConfig.Frequency.Private[newFrequency] or radioConfig.Frequency.Access[newFrequency] then
                                ...
                                if idx ~= nil then
                                    ...
                                    if Radio.On then
                                        ...
                                        --print("1")
                                        --ESX.ShowNotification("~g~~h~E ~w~pour revenir sur les canaux")
                                    end
                                end
                            end
                        end
...
function Radio:Toggle(toggle)
	...
	if self.On and PlayerData.job.name ~= "sheriff" and PlayerData.job.name ~= "police" and PlayerData.job.name ~= 'ambulance' and PlayerData.job.name ~= 'gouv' and PlayerData.job.name ~= 'fbi' and PlayerData.job.name ~= 'army' then
	...
	if self.Open then
		...
		DetachEntity(self.Handle, true, false)
        DeleteEntity(self.Handle)
        ESX.ShowNotification("Vous avez ~r~rangé~s~ votre radio.")
	end
end
...
function SetAllowRadioWhenClosed(value)
    ...
    if Radio.On and not Radio.Open and radioConfig.AllowRadioWhenClosed then
        if PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'gouv' or PlayerData.job.name == 'fbi' or PlayerData.job.name == 'army' then
      ...
      --  local isPlayingBroadcastAnim = IsEntityPlayingAnim(playerPed, "random@arrests","generic_radio_chatter", 3)
        ...
        if Radio.Open then
            ...
            if currentWeapon ~= `weapon_unarmed` then
                ...
                Radio:Toggle(false)
                Radio.On = false
                ESX.ShowNotification("~r~Vous avez rangé votre radio ~n~~g~Faite ~r~F11 ~g~pour la reprendre")
            end
            ...
            if IsControlJustPressed(0, radioConfig.Controls.Toggle.Key) then
                ESX.ShowNotification('Vous êtes sur la fréquence n° ~g~' ..radioConfig.Frequency.Current.. '~n~~n~~g~E ~w~Pour naviguer dans les fréquences radio ~n~~r~F11  ~s~pour la ranger')
                Wait(100)
      
                Radio.On = not Radio.On
        print(radioConfig.Frequency.Current)
                if Radio.On then
                    local radionum = radioConfig.Frequency.Current
                    if radionum >= 1 and radionum <= 20 then
                        if radionum <= 5 and (PlayerData.job.name == 'police' or PlayerData.job.name == 'sheriff') then
                            SendNUIMessage({ sound = "audio_on", volume = 0.3})
                            Radio:Add(radioConfig.Frequency.Current)
                            ESX.ShowNotification("Vous avez ~g~activé~s~ votre radio")
                            IsAuto = true
                            exports["mumble-voip"]:SetMumbleProperty("radioEnabled", Radio.On)
                        elseif radionum >= 6 and radionum <= 8 and PlayerData.job.name == 'ambulance' then
                            SendNUIMessage({ sound = "audio_on", volume = 0.3})
                            Radio:Add(radioConfig.Frequency.Current)
                            exports["mumble-voip"]:SetMumbleProperty("radioEnabled", Radio.On)
                            ESX.ShowNotification("Vous avez ~g~activé~s~ votre radio")
                            IsAuto = true
                        elseif radionum >= 9 and radionum <= 10 and PlayerData.job.name == 'gouv' then
                            SendNUIMessage({ sound = "audio_on", volume = 0.3})
                            Radio:Add(radioConfig.Frequency.Current)  
                            exports["mumble-voip"]:SetMumbleProperty("radioEnabled", Radio.On)
                            ESX.ShowNotification("Vous avez ~g~activé~s~ votre radio")
                            IsAuto = true
                        elseif radionum >= 11 and radionum <= 15 and PlayerData.job.name == 'fbi' then
                            SendNUIMessage({ sound = "audio_on", volume = 0.3})
                            Radio:Add(radioConfig.Frequency.Current)  
                            exports["mumble-voip"]:SetMumbleProperty("radioEnabled", Radio.On)
                            ESX.ShowNotification("Vous avez ~g~activé~s~ votre radio")
                            IsAuto = true
                        elseif radionum >= 16 and radionum <= 20 and PlayerData.job.name == 'army' then
                            SendNUIMessage({ sound = "audio_on", volume = 0.3})
                            Radio:Add(radioConfig.Frequency.Current)  
                            exports["mumble-voip"]:SetMumbleProperty("radioEnabled", Radio.On)
                            ESX.ShowNotification("Vous avez ~g~activé~s~ votre radio")
                            ...
                            ESX.ShowNotification("Vous ne ~r~pouvez pas~s~ rejoindre cette fréquence")
                           -- ESX.ShowNotification("~g~E ~w~Pour revenir dans les canaux radio")
                           -- ESX.ShowNotification("Rappuyez sur E pour pouvoir naviguer dans les fréquences")
                            IsAuto = false
                        end
                    else
                        SendNUIMessage({ sound = "audio_on", volume = 0.3})
                    ...
                    ESX.ShowNotification("Vous avez ~r~désactivé~s~ votre radio.")
                    IsAuto = false
                    Radio:Remove()
                end
            end
            ...
            if not Radio.On then
                ...
                if not radioConfig.Controls.Input.Pressed then
                    if IsControlJustPressed(0, radioConfig.Controls.Input.Key) then
                        ...
                        Citizen.CreateThread(function()
                            DisplayOnscreenKeyboard(1, "", "", radioConfig.Frequency.Current, "", "", "", 3)
                ...
                if IsDisabledControlJustPressed(0, radioConfig.Controls.ToggleClicks.Key) then
                    if Radio.Clicks then
                        ESX.ShowNotification('Vous avez ~g~activé~s~ le mode muet de votre radio')
                    else
                        ESX.ShowNotification('Vous avez ~r~desactivé~s~ le mode muet de votre radio')
                    end
            ...
            Citizen.Wait(10)
		--	if radioConfig.AllowRadioWhenClosed and (PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'gouv' or PlayerData.job.name == 'fbi') then
