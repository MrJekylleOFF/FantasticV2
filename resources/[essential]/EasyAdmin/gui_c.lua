...
Citizen.CreateThread(function()
	...
	mainMenu = NativeUI.CreateMenu("Admin Menu", "~b~Fantastic", menuOrientation, 200)
...
function GenerateMenu() -- this is a big ass function
	...
	mainMenu = NativeUI.CreateMenu("Menu Admin", "Fantastic", menuOrientation, 200,'', '',nil, 100, 0, 0, 255)
	_menuPool:Add(mainMenu)
	...
	playermanagement = _menuPool:AddSubMenu(mainMenu, GetLocalisedText("playermanagement"),"",true, 200,'', '',nil, 100, 0, 0, 255)
	servermanagement = _menuPool:AddSubMenu(mainMenu, GetLocalisedText("servermanagement"),"",true, 200,'', '',nil, 100, 0, 0, 255)
	settingsMenu = _menuPool:AddSubMenu(mainMenu, GetLocalisedText("settings"),"",true, 200,'', '',nil, 100, 0, 0, 255)
	...
	for i,thePlayer in ipairs(players) do
		thisPlayer = _menuPool:AddSubMenu(playermanagement,"ID:"..GetPlayerServerId(thePlayer).." - "..GetPlayerName(thePlayer),"",true, 200,'', '',nil, 100, 0, 0, 255)
		...
		if permissions.warning then			
			local thisItem = NativeUI.CreateItem("Avertir le joueur","Avertit le joueur")
			thisPlayer:AddItem(thisItem)
			thisItem.Activated = function(ParentMenu,SelectedItem)
				AddTextEntry("easyadmin_warning", "Message d'avertissement : ")
				DisplayOnscreenKeyboard(1, "easyadmin_warning", "", "", "", "", "", 200)
				...
				if result and result ~= "" then
					TriggerServerEvent("EasyAdmin:warningPlayer", GetPlayerServerId( thePlayer ), result)
				end
		...
		if permissions.crash then			
			local thisItem = NativeUI.CreateItem("~r~Crash le joueur","Fais crash le joueur")
			thisPlayer:AddItem(thisItem)
			thisItem.Activated = function(ParentMenu,SelectedItem)
				TriggerServerEvent("EasyAdmin:crashPlayer", GetPlayerServerId( thePlayer ))
			end
		end
	...
	CachedList = _menuPool:AddSubMenu(playermanagement,"~r~Attention","~r~Ne pas toucher",false)
