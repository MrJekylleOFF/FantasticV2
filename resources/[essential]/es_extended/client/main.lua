...
AddEventHandler('esx:playerLoaded', function(playerData)
	...
	if Config.EnableHud then
		for k,v in ipairs(playerData.accounts) do
			...
			local moneyTpl = '<div>{{money}} $</div>' 

			ESX.UI.HUD.RegisterElement('account_money', 1, 0, moneyTpl, {
			  money = 0
			})

			if v.name == 'money' then
			ESX.UI.HUD.UpdateElement('account_money', {
			  money = ESX.Math.GroupDigits(v.money)
			})
			end
...
AddEventHandler('esx:removeInventoryItem1', function(item, count, showNotification)
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item then
...
AddEventHandler('esx:createMissingPickups', function(missingPickups)
	for pickupId,pickup in pairs(missingPickups) do
		...
		if pickup.type == 'item_weapon' then
			...
			ESX.Game.SpawnLocalObject('prop_cs_cardbox_01', pickup.coords, function(obj)
				pickupObject = obj
			end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if loadoutChanged and isLoadoutLoaded then
			...
			TriggerServerEvent('esx:updateLoadout1', loadout)
		end
	end
end)
				...
				if distance < 1 then
					if IsControlJustReleased(0, 38) then
						if IsPedOnFoot(playerPed) and (closestDistance == -1 or closestDistance > 3) and not v.inRange then
							...
							TriggerServerEvent('esx:onPickup1', v.id)
