...
ESX.TriggerServerCallback = function(name, cb, ...)
	...
	TriggerServerEvent('esx:triggerServerCallback1', name, ESX.CurrentRequestId, ...)
...
ESX.ShowInventory = function()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory', {
		title    = _U('inventory',  currentWeight/1000, ESX.PlayerData.maxWeight/1000),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		menu.close()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory_item', {
							...
							if playersNearby[selectedPlayer] then
								...
								if IsPedOnFoot(selectedPlayerPed) then
									if type == 'item_weapon' then
										TriggerServerEvent('esx:giveInventoryItem1', selectedPlayerId, type, item, nil)
										menu2.close()
										menu1.close()
									else
										ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
											...
											if quantity then
												TriggerServerEvent('esx:giveInventoryItem1', selectedPlayerId, type, item, quantity)
												menu3.close()
												menu2.close()
												menu1.close()
											else
			...
			elseif data1.current.action == 'remove' then
				if IsPedOnFoot(playerPed) then
					...
					if type == 'item_weapon' then
						...
						Citizen.Wait(1000)
						TriggerServerEvent('esx:removeInventoryItem1', type, item)
					else
						ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_remove', {
							...
							if quantity then
								...
								TaskPlayAnim(playerPed, dict, anim, 8.0, 1.0, 1000, 16, 0.0, false, false, false)
								Citizen.Wait(1000)
								TriggerServerEvent('esx:removeInventoryItem1', type, item, quantity)
							else
								ESX.ShowNotification(_U('amount_invalid'))
							end
						end, function(data2, menu2)
			...
			elseif data1.current.action == 'use' then
				TriggerServerEvent('esx:useItem1', item)
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
								...
								if quantity then
									if pedAmmo >= quantity and quantity > 0 then
										TriggerServerEvent('esx:giveInventoryItem1', GetPlayerServerId(closestPlayer), 'item_ammo', item, quantity)
										menu2.close()
										menu1.close()
									else
