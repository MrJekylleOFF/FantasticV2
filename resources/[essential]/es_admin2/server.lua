...
function permBanUser(bannedBy, id)
	bannedTable[id] = {
		banner = bannedBy,
		reason = "Banni définitivement de ce serveur",
		expire = 0
	}
...
AddEventHandler('playerConnecting', function(user, set)
	for k,v in ipairs(GetPlayerIdentifiers(source))do
		...
		if banData ~= false then
			set("Banni pour: " .. banData.reason .. "\nExpire: " .. (os.date("%c", banData.expire)))
			CancelEvent()
			break
		end
	end
...
TriggerEvent('es:addGroupCommand', 'noclip', "admin", function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = {"^1SYSTEM", "Vous n'avez pas la permission de faire celas!"} })
