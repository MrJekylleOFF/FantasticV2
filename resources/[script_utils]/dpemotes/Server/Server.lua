...
if Config.SqlKeybinding then
  MySQL.ready(function()
	...
	AddEventHandler('dp:ServerKeybindExist', function()
		local src = source local srcid = PlayerIdentifier('license', source)
	...
	AddEventHandler('dp:ServerKeybindDelete', function()
	local src = source local srcid = GetPlayerIdentifier(source)
	MySQL.Async.execute("DELETE FROM dpkeybinds WHERE id = @id",
		{
		['@id'] = srcid,
		},
		function(result)

		TriggerClientEvent('esx:showNotification', src, "Vous supprimer votre animation raccourci")
	end)
	...
	AddEventHandler("dp:ServerKeybindUpdate", function(key, emote)
		local src = source local myid =  PlayerIdentifier('license', source)
