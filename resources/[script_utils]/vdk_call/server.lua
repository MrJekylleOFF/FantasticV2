...
-- Receive call event
AddEventHandler("call:makeCall", function(job, pos, msg)

    local xCaller = ESX.GetPlayerFromId(source)
    phonenumber = xCaller.get('phone_number')
    print(phonenumber)
    ...
    for _, player in pairs(inService[job]) do
        TriggerClientEvent("call:callIncoming", player, job, pos, msg, phonenumber)
    end
