...
function IsJobTrue()
    if PlayerData ~= nil then
        ...
        if PlayerData.job ~= nil and PlayerData.job.name == 'yellow' then
            IsJobTrue = true
        end
        return IsJobTrue
    end
...
function OpenSocietyActionsMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'yellow_actions',
    {
      title    = _U('yellow'),
      align    = 'top-left',
      elements = elements
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill1', GetPlayerServerId(player), 'society_yellow', _U('billing'), amount)
        end
...
function OpenGetFridgeStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_yellowjob:getFridgeStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_yellowjob:getFridgeStockItem', itemName, count)
            end
...
ESX.TriggerServerCallback('esx_yellowjob:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_yellowjob:putFridgeStockItems', itemName, count)
            end
...
function OpenShopMenu(zone)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'yellow_shop',
        {
            title    = _U('shop'),
            elements = elements
        },
        function(data, menu)
            TriggerServerEvent('esx_yellowjob:buyItem', data.current.value, data.current.price, data.current.realLabel)
        end,
        function(data, menu)
            menu.close()
        end
...
AddEventHandler('esx_yellowjob:hasEnteredMarker', function(zone)
...
AddEventHandler('esx_yellowjob:hasExitedMarker', function(zone)
    ...
    while true do
        ...
        if IsJobTrue() then
            ...
            if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
                ...
                TriggerEvent('esx_yellowjob:hasEnteredMarker', currentZone)
            end

            if not isInMarker and HasAlreadyEnteredMarker then
                HasAlreadyEnteredMarker = false
                TriggerEvent('esx_yellowjob:hasExitedMarker', LastZone)
            end
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
        ...
        if CurrentAction == 'delete_vehicle' then
            ...
            if
              GetEntityModel(vehicle) == GetHashKey('burrito3') or GetEntityModel(vehicle) == GetHashKey('mule3')
            then
              TriggerServerEvent('esx_service:disableService', 'yellow')
            end
        ...
        if CurrentAction == 'menu_boss_actions' and IsGradeBoss() then
          ...
          TriggerEvent('esx_society:openBossMenu1', 'yellow', function(data, menu)
    ...
    if IsControlJustReleased(0,  Keys['F6']) and IsJobTrue() and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'yellow_actions') then
