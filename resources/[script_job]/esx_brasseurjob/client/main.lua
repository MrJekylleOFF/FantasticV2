...
local HasAlreadyEnteredMarker, publicBlip, playerInService = false, false, false
local LastZone, CurrentAction, CurrentActionMsg  = nil, nil, ''
...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = 'arreter son service', value = 'citizen_wear'},
				{label = 'prendre son service', value = 'brasseur_wear'},
			},
		},
		function(data, menu)
					...
					if not isInService then
						...
						ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
							if not canTakeService then
								...
								TriggerServerEvent('esx_service:notifyAllInService', notification, 'brasseur')
								TriggerServerEvent('esx_service:ActivateService', 'brasseur', 99)
								TriggerServerEvent('esx_service:startPayCheck', 'brasseur', 'society_brasseur')
								blipsjob()
...
function OpenbrasseurActionsMenu()
	...
	if Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name == 'boss' then -- Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name == 'boss'
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu1', 'brasseur', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					if not IsAnyVehicleNearPoint(Config.Zones.VehicleSpawnPoint.Pos.x, Config.Zones.VehicleSpawnPoint.Pos.y, Config.Zones.VehicleSpawnPoint.Pos.z, 5.0) then
		...
		local elements = {
			{label = 'Véhicule de Travail',  value = 'Bison'}
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				if not IsAnyVehicleNearPoint(Config.Zones.VehicleSpawnPoint.Pos.x, Config.Zones.VehicleSpawnPoint.Pos.y, Config.Zones.VehicleSpawnPoint.Pos.z, 5.0) then
					...
					ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 28.7055, function(vehicle)
						SetVehicleNumberPlateText(vehicle, "BRAS" .. platenum)
						local playerPed = GetPlayerPed(-1)
						TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
						local plate = GetVehicleNumberPlateText(vehicle)
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill1', GetPlayerServerId(closestPlayer), 'society_brasseur', 'brasseur', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_brasseurjob:hasEnteredMarker', function(zone)
	if zone == 'OrgeFarm' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'orge_harvest'
		CurrentActionMsg  = _U('press_collect_orge')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'HoublonFarm' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'houblon_harvest'
		CurrentActionMsg  = _U('press_collect_houblon')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementBiereblanche' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'biereblanche_traitement'
		CurrentActionMsg  = _U('press_traitement_blanche')
		CurrentActionData = {zone = zone}
	end

	if zone == 'TraitementBierebrune' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'bierebrune_traitement'
		CurrentActionMsg  = _U('press_traitement_brune')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'brasseurActions' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur' then
		CurrentAction     = 'brasseur_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur' then
...
AddEventHandler('esx_brasseurjob:hasExitedMarker', function(zone)
	...
	if (zone == 'HoublonFarm') and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur' then
		TriggerServerEvent('esx_brasseurjob:stopHarvestHoublon')
	end  
	if (zone == 'TraitementBiereblanche' or zone == 'TraitementBierebrune') and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur' then
...
function blipsjob()
	if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  38) and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur' then
				if CurrentAction == 'orge_harvest' then
					TriggerServerEvent('esx_brasseurjob:startHarvest', CurrentActionData.zone)			
				elseif CurrentAction == 'houblon_harvest' then
					TriggerServerEvent('esx_brasseurjob:startHarvestHoublon', CurrentActionData.zone)		
				elseif CurrentAction == 'biereblanche_traitement' then
					TriggerServerEvent('esx_brasseurjob:startTransform', CurrentActionData.zone)	
				elseif CurrentAction == 'bierebrune_traitement' then
					TriggerServerEvent('esx_brasseurjob:startTransform', CurrentActionData.zone)	
				elseif CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_brasseurjob:startSell', CurrentActionData.zone)
				elseif CurrentAction == 'brasseur_actions_menu' then
		...
		if IsControlPressed(0,  167) and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'brasseur' then
