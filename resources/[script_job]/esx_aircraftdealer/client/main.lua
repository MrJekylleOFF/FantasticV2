...
Citizen.CreateThread(function()
	...
	ESX.TriggerServerCallback('esx_aircraftjob:getCategories', function(categories)
		Categories = categories
	end)

	ESX.TriggerServerCallback('esx_aircraftjob:getVehicles', function(vehicles)
		Vehicles = vehicles
	end)
	...
	if Config.EnablePlayerManagement then
		if ESX.PlayerData.job.name == 'aircraft' then
...
RegisterNetEvent('esx_aircraftjob:sendCategories')
AddEventHandler('esx_aircraftjob:sendCategories', function(categories)
	Categories = categories
end)

RegisterNetEvent('esx_aircraftjob:sendVehicles')
AddEventHandler('esx_aircraftjob:sendVehicles', function(vehicles)
	Vehicles = vehicles
end)
...
function ReturnVehicleProvider()
	ESX.TriggerServerCallback('esx_aircraftjob:getCommercialVehicles', function(vehicles)
		local elements = {}
		local returnPrice
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_provider_menu', {
			...
			TriggerServerEvent('esx_aircraftjob:returnProvider1', data.current.value)
...
function OpenShopMenu()
	...
	for i=1, #Vehicles, 1 do
		if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			...
			print(('esx_aircraftjob: vehicle "%s" does not exist'):format(Vehicles[i].model))
		end
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					ESX.TriggerServerCallback('esx_aircraftjob:buyVehicleSociety', function(hasEnoughMoney)
					...
					end, 'aircraft', vehicleData.model)
				else
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								...
								ESX.TriggerServerCallback('esx_aircraftjob:buyVehicle', function(hasEnoughMoney)
									if hasEnoughMoney then
										IsInShopMenu = false
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
											...
											if Config.EnableOwnedVehicles then
												TriggerServerEvent('esx_aircraftjob:setVehicleOwned1', vehicleProps)
											end
								...
								ESX.TriggerServerCallback('esx_aircraftjob:buyVehicleSociety', function(hasEnoughMoney)
									if hasEnoughMoney then
										...
										ESX.Game.SpawnVehicle(vehicleData.model, Config.Zones.ShopOutside.Pos, Config.Zones.ShopOutside.Heading, function(vehicle)
											...
											vehicleProps.plate = newPlate
											SetVehicleNumberPlateText(vehicle, newPlate)
											TriggerServerEvent('esx_aircraftjob:setVehicleOwnedSociety1', playerData.job.name, vehicleProps)
											ESX.ShowNotification(_U('vehicle_purchased'))
										end)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('esx_billing:sendBill1', GetPlayerServerId(closestPlayer), 'society_aircraft', _U('car_dealer'), tonumber(data2.value))
					end
				end
			end, function(data2, menu2)
				menu2.close()
		...
		elseif action == 'set_vehicle_owner_sell' then
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				TriggerServerEvent('esx_aircraftjob:sellVehicle1', model)
				TriggerServerEvent('esx_aircraftjob:addToList1', GetPlayerServerId(closestPlayer), model, newPlate)

				if Config.EnableOwnedVehicles then
					TriggerServerEvent('esx_aircraftjob:setVehicleOwnedPlayerId', GetPlayerServerId(closestPlayer), vehicleProps)
			...
			if closestPlayer == -1 or closestDistance > 3.0 then
				...
				ESX.TriggerServerCallback('esx:getOtherPlayerData', function(xPlayer)
					...
					if Config.EnableSocietyOwnedVehicles then
						TriggerServerEvent('esx_aircraftjob:setVehicleOwnedSociety1', xPlayer.job.name, vehicleProps)
						ESX.ShowNotification(_U('vehicle_set_owned', vehicleProps.plate, GetPlayerName(closestPlayer)))
					else
						ESX.ShowNotification(_U('vehicle_sold_to', vehicleProps.plate, GetPlayerName(closestPlayer)))
					end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_rent_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 5.0 then
						...
						vehicleProps.plate = newPlate
						SetVehicleNumberPlateText(LastVehicles[#LastVehicles], newPlate)
						TriggerServerEvent('esx_aircraftjob:rentVehicle1', model, vehicleProps.plate, GetPlayerName(closestPlayer), CurrentVehicleData.price, amount, GetPlayerServerId(closestPlayer))

						if Config.EnableOwnedVehicles then
							TriggerServerEvent('esx_aircraftjob:setVehicleOwnedPlayerId1', GetPlayerServerId(closestPlayer), vehicleProps)
						end
...
function OpenRentedVehiclesMenu()
	ESX.TriggerServerCallback('esx_aircraftjob:getRentedVehicles', function(vehicles)
		local elements = {}
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu1', 'aircraft', function(data2, menu2)
				menu2.close()
			end)
		elseif data.current.value == 'sold_vehicles' then

			ESX.TriggerServerCallback('esx_aircraftjob:getSoldVehicles', function(customers)
				local elements = {
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('esx_aircraftjob:getStockItems1', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_aircraftjob:getStockItem1', itemName, count)
					menu2.close()
					menu.close()
					OpenGetStocksMenu()
				end
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('esx_aircraftjob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					TriggerServerEvent('esx_aircraftjob:putStockItems1', itemName, count)
					menu2.close()
					menu.close()
					OpenPutStocksMenu()
				end
...
AddEventHandler('esx_aircraftjob:hasEnteredMarker', function(zone)
	if zone == 'ShopEntering' then

		if Config.EnablePlayerManagement then
			if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'aircraft' then
				CurrentAction     = 'reseller_menu'
				CurrentActionMsg  = _U('shop_menu')
				CurrentActionData = {}
			end
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'aircraft' and ESX.PlayerData.job.grade_name == 'boss' then
...
AddEventHandler('esx_aircraftjob:hasExitedMarker', function(zone)
...
Citizen.CreateThread(function()
	while true do
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			TriggerEvent('esx_aircraftjob:hasEnteredMarker', currentZone)
		end

		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_aircraftjob:hasExitedMarker', LastZone)
		end
