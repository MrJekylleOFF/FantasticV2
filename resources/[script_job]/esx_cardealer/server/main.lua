...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('[orion_vehicleshop] [^3WARNING^7] Plate character count reached, %s/8 characters!'):format(char))
	end
end)
...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT * FROM vehicle_categories', {}, function(_categories)
		...
		MySQL.Async.fetchAll('SELECT * FROM vehicles', {}, function(_vehicles)
			...
			TriggerClientEvent('orion_vehicleshop:sendCategories', -1, categories)
			TriggerClientEvent('orion_vehicleshop:sendVehicles', -1, vehicles)
		end)
	end)
end)
...
RegisterNetEvent('orion_vehicleshop:setVehicleOwnedPlayerId')
AddEventHandler('orion_vehicleshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
...
ESX.RegisterServerCallback('orion_vehicleshop:getSoldVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT client, model, plate, soldby, date FROM vehicle_sold', {}, function(result)
		cb(result)
	end)
end)

RegisterNetEvent('orion_vehicleshop:rentVehicle')
AddEventHandler('orion_vehicleshop:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)
...
RegisterNetEvent('orion_vehicleshop:getStockItem')
AddEventHandler('orion_vehicleshop:getStockItem', function(itemName, count)
...
RegisterNetEvent('orion_vehicleshop:putStockItems')
AddEventHandler('orion_vehicleshop:putStockItems', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('orion_vehicleshop:getCategories', function(source, cb)
	cb(categories)
end)

ESX.RegisterServerCallback('orion_vehicleshop:getVehicles', function(source, cb)
	cb(vehicles)
end)

ESX.RegisterServerCallback('orion_vehicleshop:buyVehicle', function(source, cb, model, plate)
...
ESX.RegisterServerCallback('orion_vehicleshop:getCommercialVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT price, vehicle FROM cardealer_vehicles ORDER BY vehicle ASC', {}, function(result)
		cb(result)
	end)
end)

ESX.RegisterServerCallback('orion_vehicleshop:buyCarDealerVehicle', function(source, cb, model)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('orion_vehicleshop:returnProvider')
AddEventHandler('orion_vehicleshop:returnProvider', function(vehicleModel)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'cardealer' then
		MySQL.Async.fetchAll('SELECT id, price FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			...
			if result[1] then
				...
				print(('[orion_vehicleshop] [^3WARNING^7] %s attempted selling an invalid vehicle!'):format(xPlayer.identifier))
			end
		end)
	end
end)

ESX.RegisterServerCallback('orion_vehicleshop:getRentedVehicles', function(source, cb)
...
ESX.RegisterServerCallback('orion_vehicleshop:giveBackVehicle', function(source, cb, plate)
...
ESX.RegisterServerCallback('orion_vehicleshop:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'cardealer' then
		...
		if not resellPrice then
			print(('[orion_vehicleshop] [^3WARNING^7] %s attempted to sell an unknown vehicle!'):format(xPlayer.identifier))
			cb(false)
		else
						...
						if result[1] then -- does the owner match?
							...
							if vehicle.model == model then
								if vehicle.plate == plate then
									...
									print(('[orion_vehicleshop] [^3WARNING^7] %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
									cb(false)
								end
							else
								print(('[orion_vehicleshop] [^3WARNING^7] %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
								cb(false)
							end
						end
					end)
...
ESX.RegisterServerCallback('orion_vehicleshop:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('orion_vehicleshop:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('orion_vehicleshop:isPlateTaken', function(source, cb, plate)
...
ESX.RegisterServerCallback('orion_vehicleshop:retrieveJobVehicles', function(source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('orion_vehicleshop:setJobVehicleState')
AddEventHandler('orion_vehicleshop:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[orion_vehicleshop] [^3WARNING^7] %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
...
function PayRent(d, h, m)
	...
	print('[orion_vehicleshop] [^2INFO^7] Paying rent cron job started')
	...
	MySQL.Async.fetchAll('SELECT owner, rent_price FROM rented_vehicles', {}, function(result)
		...
		print(('[orion_vehicleshop] [^2INFO^7] Paying rent cron job took %s seconds'):format(elapsedTime))
	end)
end
