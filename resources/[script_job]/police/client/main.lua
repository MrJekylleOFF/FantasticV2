...
function OpenCloakroomMenu()
	...
	if grade == 'cadet' then
		table.insert(elements, {label = 'Tenue de service', value = 'cadet_wear'})
	elseif grade == 'officier' then
		table.insert(elements, {label = 'Tenue de service', value = 'officier_wear'})
	elseif grade == 'officierI' then
		table.insert(elements, {label = 'Tenue de service', value = 'officierI_wear'})
	elseif grade == 'officierII' then
		table.insert(elements, {label = 'Tenue de service', value = 'officierII_wear'})
	elseif grade == 'officierIII' then
		table.insert(elements, {label = 'Tenue de service', value = 'officierIII_wear'})
	elseif grade == 'officiersenoir' then
		table.insert(elements, {label = 'Tenue de service', value = 'officiersenoir_wear'})
	elseif grade == 'Sergent' then
		table.insert(elements, {label = 'Tenue de service', value = 'Sergent_wear'})
	elseif grade == 'SergentII' then
		table.insert(elements, {label = 'Tenue de service', value = 'SergentII_wear'})
	elseif grade == 'lieutenant' then
		table.insert(elements, {label = 'Tenue de service', value = 'lieutenant_wear'})
	elseif grade == 'lieutenantII' then
			table.insert(elements, {label = 'Tenue de service', value = 'lieutenantII_wear'})
	elseif grade == 'captaine' then
			table.insert(elements, {label = 'Tenue de service', value = 'captaine_wear'})
	elseif grade == 'boss' then
			table.insert(elements, {label = 'Tenue de service', value = 'boss_wear'})
	elseif grade == 'agentsecret' then
			table.insert(elements, {label = 'Tenue de service', value = 'agentsecret_wear'})
	elseif grade == 'chef' then
			table.insert(elements, {label = 'Tenue de service', value = 'che_wear'})
	end
			...
			if Config.MaxInService ~= -1 then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'bav')
						TriggerServerEvent('esx_service:disableService', 'bav')
						TriggerEvent('policejob:updateBlip')
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('policejob:storeNearbyVehicle', function(storeSuccess, foundNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('policejob:buyJobVehicle', function (bought)
					if bought then
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('policejob:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
...
function OpenIdentityCardMenu(player)
	ESX.TriggerServerCallback('policejob:getOtherPlayerData', function(data)
		local elements = {}
	...
	ESX.TriggerServerCallback('policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('policejob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle',
	{
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('policejob:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('policejob:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenBuyWeaponsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		...
		if data.current.hasWeapon then
			...
			ESX.TriggerServerCallback('policejob:buyWeapon', function(bought)
				if bought then
...
function OpenGetStocksMenu()
	ESX.TriggerServerCallback('policejob:getStockItems', function(items)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('policejob:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	ESX.TriggerServerCallback('policejob:getPlayerInventory', function(inventory)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('policejob:putStockItems', itemName, count)
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if PlayerData.job and PlayerData.job.name == 'bav' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('policejob:hasEnteredEntityZone', function(entity)
	...
	if PlayerData.job and PlayerData.job.name == 'bav' and IsPedOnFoot(playerPed) then
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job and PlayerData.job.name == 'bav' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'bav' then
				...
				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openBossMenu1', 'bav', function(data, menu)
						menu.close()
	...
	if PlayerData.job and PlayerData.job.name == 'bav' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'bav' then
...
Citizen.CreateThread(function()
	while true do 
		Citizen.Wait(5)
		if PlayerData.job ~= nil and PlayerData.job.name == 'bav' then  
...
AddEventHandler('playerSpawned', function(spawn)
	...
	if not hasAlreadyJoined then
		TriggerServerEvent('policejob:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('policejob:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'bav')
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        _menuPool:MouseEdgeEnabled (false);
            ...
            if IsEntityAtCoord(PlayerPedId(), 638.28, -2.37, 82.79, 1.5, 1.5, 1.5, 0, 1, 0) then 
...
spawnCar = function(car)
    ...
    local vehicle = CreateVehicle(car, 649.62, -10.76, 82.79, 227.32, true, false)
    SetEntityAsMissionEntity(vehicle, true, true)
    SetVehicleNumberPlateText(vehicle, "BAV ROULE")
end