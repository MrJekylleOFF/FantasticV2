...
--
---- CHANGER OWNER
RegisterServerEvent('esx_vehiclelock:changeownerkey')
AddEventHandler('esx_vehiclelock:changeownerkey', function(target, plate)
local xPlayertarget = ESX.GetPlayerFromId(target)
local toplate = plate
print(xPlayertarget.identifier .. ' - ' .. plate)
MySQL.Async.fetchAll(
		...
		for i=1, #result, 1 do
			MySQL.Async.execute(
						'UPDATE owned_vehicles SET owner = @owner WHERE id = @id',
						{
							['@id'] = result[i].id,
...
RegisterServerEvent('esx_vehiclelock:donnerkey')
AddEventHandler('esx_vehiclelock:donnerkey', function(target, plate, vehicleProps)
local _source = source
local xPlayer = nil
local toplate = plate
--local vehicleProps.plate = plate 
...
local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    function(result)
        ...
        TriggerClientEvent('esx:showAdvancedNotification', xPlayertarget.source, '~r~Fantastic', 'Centralisation', '~g~Tu as reçu les clefs du véhicules (' .. plate .. ')', 'CHAR_MP_MORS_MUTUAL', 1)
        TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, '~r~Fantastic', 'Centralisation', '~g~Tu as donner les clefs du véhicules (' .. plate .. ')', 'CHAR_MP_MORS_MUTUAL', 1)
