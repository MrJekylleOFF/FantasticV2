	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Utilisez ~y~ZQSD~s~ pour deplacer le bloc de gauche et ~y~LES FLECHES~s~ pour le bloc de droite")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("~y~ESPACE~s~ pour valider le bloc de gauche et avec ~y~ENTRER~s~ pour valider le bloc de droite")
			else
