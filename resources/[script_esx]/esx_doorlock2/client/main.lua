...
AddEventHandler('esx:setJob2', function(job) ESX.PlayerData.job2 = job2 end)
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in ipairs(Config.DoorList) do
			...
			v.isAuthorized2 = isAuthorized2(v)
	...
	while true do
		...
		for k,v in ipairs(Config.DoorList) do
			...
			if v.distanceToPlayer and v.distanceToPlayer < v.maxDistance then
				...
				if v.isAuthorized or v.isAuthorized2 then displayText = _U('press_button', displayText) end
				...
				if IsControlJustReleased(0, 38) then
					...
					if v.isAuthorized or v.isAuthorized2 then
