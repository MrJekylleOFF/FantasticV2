...
TriggerEvent('esx:getSharedObject', function(obj)
  ...
  function notifyAlertSMS (number, alert, listSrc, ano)
    
  
    if PhoneNumbers[number] ~= nil then
      for k, _ in pairs(listSrc) do
        getPhoneNumber(tonumber(k), function (n)
          if n ~= nil and not ano then
            ...
            if alert.coords ~= nil and not ano then
          ...
          if ano then
            TriggerEvent('gcPhone:_internalAddMessage', number, n, '#Anonyme : '..alert.message..'', 0, function (smsMess)
              TriggerClientEvent("gcPhone:receiveMessage", tonumber(k), smsMess)
            end)
            TriggerEvent('gcPhone:_internalAddMessage', number, n, 'GPS: ' .. alert.coords.x .. ', ' .. alert.coords.y, 0, function (smsMess)
              TriggerClientEvent("gcPhone:receiveMessage", tonumber(k), smsMess)
...
RegisterServerEvent('esx_addons_gcphone:startAnonymeCall')
AddEventHandler('esx_addons_gcphone:startAnonymeCall', function (number, message, coords)
  local source = source
  if PhoneNumbers[number] ~= nil then
    getPhoneNumber(source, function (phone) 
      notifyAlertSMS(number, {
        ...
        coords = coords,
        numero = phone,
      }, PhoneNumbers[number].sources, true)
    end)
  else
